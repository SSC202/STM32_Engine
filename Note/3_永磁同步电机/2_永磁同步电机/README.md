...
- 按照感应电动势的波形分类，永磁同步电机分为**永磁同步电机(PMSM，感应电动势为正弦波)**，**直流无刷电机(BLDC，感应电动势为梯形波)**。二者的区别在于永磁体的安装方式/绕组方式不同，产生的反电动势不同，这里不做过多讨论。

| **不同点** | **BLDC**                                     | **PMSM**                                     |
| ---------- | -------------------------------------------- | -------------------------------------------- |
| 反电动势   | 具有梯形反电动势                             | 具有正弦波反电动势                           |
| 运行电流   | 梯形波电流                                   | 正弦波电流                                   |
| 绕组方式   | 定子绕组为**集中绕组**，永磁转子形成方波磁场 | 定子绕组为**分布绕组**，永磁转子形成正弦磁场 |
| 控制方式   | 控制算法相对简单（可用六步换相/FOC/DTC）     | 控制算法复杂（可用FOC/DTC）                  |
