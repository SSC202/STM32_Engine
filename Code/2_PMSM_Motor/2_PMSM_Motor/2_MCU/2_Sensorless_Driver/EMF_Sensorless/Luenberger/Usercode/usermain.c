...
void usermain()
{
    ...
    // SMO 初始化
    Luenberger_Init(&lun, -4, 14, 10000, 20000, 10, system_sample_time);
    // 编码器初始化
    ...
    while (1)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            printf("i_alpha:%.4f,%.4f\r\n", i_alpha_lpf.output, lun.i_hat.alpha);
            break;
        case 3:
            printf("i_beta:%.4f,%.4f\r\n", i_beta_lpf.output, lun.i_hat.beta);
            break;
        case 4:
            printf("theta:%.4f,%.4f\r\n", lun.theta_true, lun.theta_obs);
            break;
        case 5:
            printf("theta:%.4f,%.4f\r\n", lun.speed_true, lun.speed_obs);
            break;
        default:
            break;
        }
...
void HAL_ADCEx_InjectedConvCpltCallback(ADC_HandleTypeDef *hadc)
{
    ...
    lun.speed_true = encoder.encoder_speed * POLE_PAIRS * encoder.encoder_direct;
    lun.theta_true = encoder.electric_theta;
    lun.u.alpha = u_alpha_beta.alpha;
    lun.u.beta = u_alpha_beta.beta;
    lun.i.alpha = i_alpha_beta.alpha;
    lun.i.beta = i_alpha_beta.beta;
    Luenberger_Update(&lun);
